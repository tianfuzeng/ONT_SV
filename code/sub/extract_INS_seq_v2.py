...
########################################################################################
# 20241109
# debug: remove INS on HBV genome; remove unmmaped reads
########################################################################################
import os
import pysam
import itertools
    ...
    with open(out_fa, 'w') as out:
        for name in n:
            try:
                ...
                for x in iterator:
                    if not (x.is_secondary or x.is_unmapped):  # secondary aligment didn't have SEQ info
                        ...
                        if len(ins_loc_list) != '0':
                            for ins_loc in ins_loc_list:
                                ...
                                if 'chr' in ins_loc[4]:  # 20241109 debug
